...
function sendMsg(firstline, msg, to) 
	TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.45vw; margin: 0.5vw; background-color: rgba(43, 43, 43, 0.6); width = auto; border-radius:3px; border: 0.5px solid rgba(255, 0, 0, 1.0);"><i class=""></i> {0} {1}</div>',
		args = { firstline , msg}
	})
end 
	...
	if not string.find(args[1], "/") and setContains(roleTracker, GetPlayerIdentifiers(source)[1]) and 
		...
		if not dontSend then
			...
			if sendBlockMessages then 
				...
				TriggerEvent('DiscordBot:ToDiscord', 'https://discord.com/api/webhooks/922623188263911475/2pxEOTFg8eT4Hb5e8bvI1nbtMEmvBWcEi8cFdLo8k9IA__-5lsBmZR9sE-lvRgDrSWgN', GetPlayerName(source) .. ' [ID: ' .. source .. '](' ..steamhex.. ')', '```'..finalMessage..'```', AvatarURL, false, Source, TTS) --Sending the message to discord
			else 
				TriggerClientEvent('chatMessage', -1, roleStr .. name .. "^7 " .. finalMessage);
			end 
		end
...
RegisterCommand('tweet', function(source, args, rawCommand)
    ...
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.45vw; margin: 0.5vw; background-color: rgba(0, 134, 237, 0.6); width = auto; border-radius:3px; border: 0.5px solid rgba(0, 0, 0, 1.0);"><i class="fab fa-twitter"></i> {0} {1}</div>',
        args = { fal, msg }
    })
 ...
 RegisterCommand('ad', function(source, args, rawCommand)
	...
	fal2 = name.firstname .. " " .. name.lastname
    TriggerClientEvent('chat:addMessage', -1, {
        template = '<div style="padding: 0.45vw; margin: 0.5vw; background-color: rgba(255, 200, 0, 0.6); width = auto; border-radius:3px; border: 0.5px solid rgba(0, 0, 0, 1.0);"><i class="fas fa-ad"></i> {0} {1}</div>',
        args = { fal, msg }
    })
